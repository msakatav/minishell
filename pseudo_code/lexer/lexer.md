...
## 関数: restore_spaces
- **概要**:
  - 文字列内の特殊文字（ASCII 1-6）を元の空白文字に戻す。
- **引数**:
  - `str`: 文字列
- **処理**:
...
## 関数: add_split_tokens
- **概要**:
  - 文字列を空白で分割し、それぞれの部分を新しいトークンとしてリストに追加する。
- **引数**:
...
- **概要**:
  - 単語トークンを処理し、必要に応じて分割や空トークンの追加を行う。
- **引数**:
  - `input`: 入力文字列
  - `i`: 現在のインデックスへのポインタ
  - `tokens`: トークンリストへのポインタ
...
## 関数: process_token
- **概要**:
  - 現在の文字に基づいて、パイプ、リダイレクト、または単語トークンを処理する。
- **引数**:
  - `input`: 入力文字列
  - `i`: 現在のインデックスへのポインタ
  - `tokens`: トークンリストへのポインタ
