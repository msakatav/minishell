...
## 関数: try_path
- **概要**:
  - ディレクトリとコマンド名を結合し、実行可能かどうかを確認する。
- **引数**:
  - `dir`: ディレクトリパス
  - `cmd`: コマンド名
- **戻り値**:
...
## 関数: search_in_paths
- **概要**:
  - PATH環境変数の各ディレクトリを検索して、実行可能なコマンドを探す。
- **引数**:
  - `paths`: パスの配列
  - `cmd`: コマンド名
- **戻り値**:
...
- **概要**:
  - カレントディレクトリ内のコマンド（`./`付き）が実行可能か確認する。
- **引数**:
  - `cmd`: コマンド名
- **戻り値**:
  - 成功時: `./` 付きの実行可能なパス
...
## 関数: find_executable
- **概要**:
  - コマンドの実行可能なパスを検索する（絶対パス、相対パス、PATH環境変数の順）。
- **引数**:
  - `cmd`: コマンド名
  - `env`: 環境変数リスト
- **戻り値**:
