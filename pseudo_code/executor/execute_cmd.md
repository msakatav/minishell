...
- **戻り値**:
  - 有効なコマンドパス
- **処理**:
  1. `find_executable` でコマンドのパスを検索する。
  2. パスが見つからない場合:
     - エラーメッセージ "command not found" を出力する。
     - `cleanup_data` を呼び出し、終了ステータス `127` で `exit` する。
  3. 実行権限がない場合 (`access(path, X_OK) != 0`):
     - エラーメッセージ "Permission denied" を出力する。
     - パスを解放し、`cleanup_data` を呼び出し、終了ステータス `126` で `exit` する。
  4. パスを返す。

## 関数: execute_external_cmd
- **概要**:
  - 外部コマンドを実行する（パス検索、権限チェック、execve）。
- **引数**:
  - `cmd`: 実行するコマンド構造体
  - `data`: シェルの主要データ構造体
- **処理**:
  1. `check_cmd_path` を呼び出し、コマンドの絶対パスを取得する。
  2. `env_to_array` を呼び出し、環境変数を配列形式に変換する。
     - 失敗した場合、`cleanup_data` を呼び出し、`exit(1)` する。
  3. `execve` を呼び出し、コマンドを実行する。
  4. `execve` が失敗した場合:
     - エラーメッセージを出力する。
     - パスと環境変数配列のメモリを解放する。
...
- **概要**:
  - 子プロセス内でリダイレクトとシグナルを設定し、外部コマンドを実行する。
- **引数**:
  - `cmd`: 実行するコマンド構造体
  - `data`: シェルの主要データ構造体
- **処理**:
...
- **概要**:
  - 親プロセスで子プロセスの終了を待ち、終了ステータスを更新する。
- **引数**:
  - `pid`: 子プロセスのPID
  - `data`: シェルの主要データ構造体
- **処理**:
